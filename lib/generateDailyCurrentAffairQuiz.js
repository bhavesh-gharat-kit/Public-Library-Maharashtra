...
function getBaseUrl(date = new Date()) {
  const monthNames = [
    "january",
    "february",
    "march",
  ...
  const month = monthNames[date.getMonth()]; // e.g. september
  const year = date.getFullYear(); // e.g. 2025

  return `https://www.gktoday.in/quizbase/current-affairs-quiz-${month}-${year}`;
}
...
export async function generateCurrentAffairQuestions(examCategoryId) {
  ...
  if (data?.length > 5) {
    ...
    await handleCreateCurrentAffairsTest(examCategoryId);
  }
}
