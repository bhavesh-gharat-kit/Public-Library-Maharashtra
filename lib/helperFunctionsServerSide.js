...
Rules:
- The Questions must not include any specific year which will show times like 2023, or any month etc.
...
export const generateFromPrompt = async (prompt, options = {}) => {
  ...
  try {
    ...
    console.log("Response is -------------------------------", rawContent);
...
export const getIBookPrompt = (bookData, contentType) => {
  ...
  const getIBookBaseContext = (bookData = {}, extra = {}) => {
    ...
    const outLang = normalizeLanguage(language||medium);

    const meta = `**Book:** ${title ? `**Book:** ${title}` : ""}  
${author ? `**Author:** ${author}` : ""}  
${outLang ? `**Language (target output):** ${outLang}` : ""}  
...
export async function insertQuestionsByCategory(examCategoryId, questions) {
  try {
    ...
    for (const q of questions) {
      ...
      await prisma.mockTestAnswer.upsert({
