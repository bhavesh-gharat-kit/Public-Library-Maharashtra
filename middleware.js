...
async function fetchAccessSettingsFromDB() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/access-settings`,
    {
...
export async function middleware(request) {
  ...
  // 2. Try to authenticate user first (so we know role)
  let user = null;
  try {
    user = await authenticate(request);
  } catch (_) {
    // ignore, user might be null if unauthenticated
  }

  // 3. Skip IP check if public access allowed OR user is admin/superadmin
  const skipIpCheck =
  ...
  // 5. Pass request with optional user info
