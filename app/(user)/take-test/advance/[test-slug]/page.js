...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchData(paramSlug) {
      ...
      try {
        setLoading(true);
        const res = await axios.get(`/api/user/tests/${paramSlug}/advance/fetch`);
        if (!res.status == 200)
  ...
  const handleMonthClick = async (monthIndex, year) => {
    ...
    try {
      ...
      const res = await axios.get(
        `/api/user/tests/${testSlug}/advance/fetch/${formattedMY}`
      );
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto w-full">
                {selectedMonthData.map((e, i) => (
                  ...
                  >
                    ...
                    <Link
                      href={`/take-test/advance/${testSlug}/play/${e.id}`}
