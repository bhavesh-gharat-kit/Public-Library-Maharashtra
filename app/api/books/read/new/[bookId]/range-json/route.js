...
// return new Response(JSON.stringify({ success: false, message: 'Extracted PDF too large for data URL. Use file endpoint.' }), { status: 413, headers: { 'Content-Type': 'application/json' } });
// }
// const dataUrl = `data:application/pdf;base64,${cached.bytes.toString('base64')}`;
// return new Response(JSON.stringify({ success: true, url: dataUrl, startPage, endPage, totalPages: cached.totalPages, size: cached.bytes.length }), { status: 200, headers: { 'Content-Type': 'application/json' } });
// }
...
// return new Response(JSON.stringify({ success: false, message: 'Extracted PDF too large for data URL. Use file endpoint.', size: bytes.length }), { status: 413, headers: { 'Content-Type': 'application/json' } });
// }


// const dataUrl = `data:application/pdf;base64,${bytes.toString('base64')}`;
// return new Response(JSON.stringify({ success: true, url: dataUrl, startPage, endPage, totalPages, size: bytes.length }), { status: 200, headers: { 'Content-Type': 'application/json' } });
// } catch (err) {
...
export async function GET(req, ctx) {
  try {
    ...
    // ✅ FIX: always convert to proper string
    const encoded = safeToString(bookId);
